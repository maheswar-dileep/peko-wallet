...
export const addAmount = async (req, res) => {
    try {
        ...
        const user = await User.updateOne({ _id: value.id }, {
...
export const transferAmount = async (req, res) => {
    try {
        ...
        if (!match)
            return res.status(406).send({ err: 'password' });
        ...
        await User.updateOne({ _id: value.id }, {
            $inc: { balance: -value.amount },
        ...
        const user = await User.updateOne({ uniqueId: value.uniqueId }, {
            $inc: { balance: value.amount },
            $push: { transactionHistory: transaction },
        });
